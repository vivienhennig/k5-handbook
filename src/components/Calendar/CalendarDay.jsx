...
export default function CalendarDay({ date, events, isToday, onClickEvent }) {
    // 1. Logik-Check: Verhindert Fehler bei leeren Tagen (Offset)
    if (!date) {
        return (
    ...
    return (
        <div className="p-3 border-b border-r border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 group hover:bg-blue-50/20 dark:hover:bg-blue-900/10 transition-colors min-h-[140px]">
            <div className="flex justify-between items-start mb-3">
                {/* CI-Anpassung: font-black für Aeonik Bold, kein italic */}
            ...
            <div className="space-y-1.5">
                {/* 2. Logik-Check: Mapping der Events */}
                {events.map(evt => (
                    <button 
                        key={evt.id} 
                        // WICHTIG: Ruft exakt die Funktion auf, die in CalendarView definiert ist
