...
export default function Sidebar({ 
    ...
    // Zentrale Navigations-Logik für die Suche
    ...
    const NavItem = ({ id, icon: Icon, label, colorClass = "text-gray-400", isWiki = false }) => {
        ...
        return (
            ...
            >
                <IconComponent size={20} className={isActive ? 'text-white' : colorClass} />
                <span className={`text-xs font-black uppercase tracking-widest italic ${isActive ? 'text-white' : 'dark:text-gray-400'}`}>
                    {label}
                </span>
    ...
    return (
        <aside className="w-72 h-screen sticky top-0 bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 p-6 flex flex-col font-sans overflow-hidden">
            ...
            <div className="mb-6 px-4 shrink-0">
                <div className="flex items-center gap-3 mb-1 cursor-pointer" onClick={() => navigate('/home')}>
            ...
            {/* NEU: WikiSearch als Command Center (Fest oben) */}
            ...
            <div className="mt-auto pt-4 border-t border-gray-100 dark:border-gray-800 space-y-3 shrink-0">
                <div className="flex items-center gap-2">
                    <button onClick={onOpenProfile} className="flex-1 flex items-center gap-3 p-3 rounded-[1.5rem] transition-all group border border-transparent hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <div className="min-w-0 text-left flex-1">
                            <p className="text-[10px] font-black uppercase text-gray-900 dark:text-white truncate">{user?.displayName || "K5 Member"}</p>
                            <p className="text-[9px] font-bold text-gray-400 uppercase italic">Profil</p>
                        </div>
                    </button>
                    <ThemeToggle />
                </div>
                <button onClick={onLogout} className="w-full flex items-center gap-3 px-4 py-2 text-gray-400 hover:text-red-500 transition-all group">
