    ...
    return (
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 border-b border-gray-100 dark:border-gray-800 pb-8 gap-6">
            <div className="flex-1 min-w-0">
                ...
                <h2 className="text-4xl lg:text-5xl font-black text-gray-900 dark:text-white tracking-tight italic">{title}</h2>
                {isEditing ? (
                    <textarea 
                    ...
                    />
                ) : <p className="text-gray-500 dark:text-gray-400 mt-4 text-lg leading-relaxed max-w-2xl">{introText}</p>}
            </div>
            ...
            <div className="flex items-center gap-3 print:hidden shrink-0">
                ...
                {isPrivileged && (
                    <button 
                        ...
                        className={`px-8 py-3 rounded-2xl font-black flex items-center gap-2 shadow-xl transition-all active:scale-95 ${isEditing ? 'bg-green-600 text-white' : 'bg-blue-600 text-white'}`}
                    >
